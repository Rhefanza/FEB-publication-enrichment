...
OUTPUT_FILE = "services_sinta_2020_2026.xlsx"
...
def parse_services_page(html):
    soup = BeautifulSoup(html, "html.parser")
    rows = []

    service_items = soup.select(".ar-list-item")
    ...
    for item in service_items:
        ...
        for meta in meta_blocks:
            ...
            if "Leader" in meta_text:
                leader_match = re.search(r"Leader\s*:\s*(.*?)(?=Pengabdian|PENGABDIAN|$)", meta_text, re.I)
                if leader_match:
            ...
            for q in quartile_tags:
                ...
                if "Rp" in q_text:
                    ...
                    status = q_text
                elif "SOURCE" in q_text.upper() or "BIMA" in q_text.upper() or "INTERNAL" in q_text.upper():
                    source = q_text

        # Fallback Leader
        if not leader:
            leader_match = re.search(r"Leader\s*:\s*(.*?)(Pengabdian|PENGABDIAN|$)", item_text, re.I)
            if leader_match:
                leader = clean_text(leader_match.group(1))
        ...
        if not members:
            members_match = re.search(
                r"Personils?\s*:\s*(.*?)(\b20\d{2}\b|Rp\.?|Approved|Rejected|BIMA|INTERNAL|SOURCE|$)",
                item_text,
                re.I
            )
            if members_match:
...
def scrape_services_by_sinta_id(page, sinta_id):
    services_url = f"{BASE_URL}/authors/profile/{sinta_id}/?view=services"
    ...
    for page_no in range(1, 50):
        print(f"  Ambil Services page {page_no}")
    ...
    with sync_playwright() as p:
        ...
        for idx, author_name in enumerate(authors, start=1):
            ...
            try:
                ...
                rows = scrape_services_by_sinta_id(
                    page=page,
                    sinta_id=sinta_id
                )

                if not rows:
                    print(f"Tidak ada Services 2020-2026: {author_name}")
                    continue

                all_rows.extend(rows)
                print(f"Services diambil: {len(rows)}")
